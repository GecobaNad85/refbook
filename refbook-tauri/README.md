...
- **系统托盘 + 悬浮图标**：托盘菜单（显示主窗口 / 划词查询 / CNKI 登录 / 悬浮图标开关 / 退出）；主窗口关闭时显示悬浮图标入口，点击唤起主窗口，可拖动并记忆位置。
...
- **Wayland 下窗口位置**：GNOME/mutter 忽略客户端 `set_position`，悬浮图标初始位置与位置记忆在 Wayland 下可能不生效（X11 正常），协议层限制。
